...
# from setup_cifar10_model import CIFAR10  # 禁用CIFAR10模型

"""##L2 Black Box Attack (Targeted, Newton, MNIST)"""
...
def l2_attack(input, target, model, targeted, use_log, use_tanh, solver, reset_adam_after_found=True,abort_early=True,
  ...
  def compare(x,y):
    if not isinstance(x, (float, int, np.int64)): # type: ignore
      if targeted:
        x[y] -= confidence
      else:
        x[y] += confidence
  ...
  for step in range(binary_search_steps):
    ...
    if compare(bestscore,  np.argmax(target.cpu().numpy(),-1)) and bestscore != -1:
      ...
      if upper_bound < 1e9:
          ...
          const *= 10
      print('new constant: ', const)
  # 确保out_best_attack的形状与输入一致
  if out_best_attack.shape != input.shape:
      # 如果缺少通道维度，添加通道维度
      if len(out_best_attack.shape) == 3 and len(input.shape) == 4:
          out_best_attack = np.expand_dims(out_best_attack, axis=1)
...
if __name__=='__main__':
  ...
  test_set = datasets.MNIST(root = './data', train=False, transform = transform, download=True)  # 启用MNIST数据集
  # test_set = datasets.CIFAR10(root = './data', train=False, transform = transform, download=True)  # 禁用CIFAR10
  test_loader = torch.utils.data.DataLoader(test_set,batch_size=1,shuffle=True)

  use_cuda=True
  device = torch.device("cuda" if (use_cuda and torch.cuda.is_available()) else "cpu")

  model = MNIST().to(device)  # 使用MNIST模型
  # model = CIFAR10().to(device)  # 禁用CIFAR10模型

  model.load_state_dict(torch.load('./models/mnist_model.pt'))  # 加载MNIST权重
  # model.load_state_dict(torch.load('./models/cifar10_model.pt'))  # 禁用CIFAR10权重
  model.eval()

  use_log=True
  use_tanh=True
  targeted=True   # 有目标攻击
  solver="newton" # 使用Newton优化器
  ...
  def save(img, filename):
    ...
    Image.fromarray(img.squeeze()).save(os.path.join("images", filename))
  ...
  if targeted:
    if solver=="newton":
      plt.savefig('newton_targeted_mnist.png')  # 保存有目标Newton攻击结果
    else:
