...
# from setup_mnist_model import MNIST  # 禁用MNIST模型
from setup_cifar10_model import CIFAR10

"""##L2 Black Box Attack (Untargeted, Adam, CIFAR10)"""
...
if __name__=='__main__':
  ...
  # test_set = datasets.MNIST(root = './data', train=False, transform = transform, download=True)  # 禁用MNIST
  test_set = datasets.CIFAR10(root = './data', train=False, transform = transform, download=True)  # 启用CIFAR10
  test_loader = torch.utils.data.DataLoader(test_set,batch_size=1,shuffle=True)

  use_cuda=True
  device = torch.device("cuda" if (use_cuda and torch.cuda.is_available()) else "cpu")

  # model = MNIST().to(device)  # 禁用MNIST模型
  model = CIFAR10().to(device)   # 启用CIFAR10模型

  # model.load_state_dict(torch.load('./models/mnist_model.pt'))  # 禁用MNIST权重
  model.load_state_dict(torch.load('./models/cifar10_model.pt'))  # 加载CIFAR10权重
  model.eval()
  ...
  def save(img, filename):
    ...
    Image.fromarray(img).save(os.path.join("images", filename))
  ...
  if targeted:
    ...
    if solver=="newton":
      ...
      plt.savefig('adam_untargeted_cifar10.png')  # 保存无目标Adam攻击结果